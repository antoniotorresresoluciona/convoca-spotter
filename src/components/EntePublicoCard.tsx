...
export function EntePublicoCard({ ente, onEdit, onDelete, onViewDetails }: EntePublicoCardProps) {
  const [isExpanded, setIsExpanded] = useState(false);
  const [isEditingUrl, setIsEditingUrl] = useState(false);
  const [editedUrl, setEditedUrl] = useState(ente.url);
  ...
  const config = statusConfig[ente.status] || statusConfig.pending;
  const StatusIcon = config.icon;

  const updatedSublinks = ente.sublinks?.filter(s => s.enabled && s.status === 'updated').length || 0;

  return (
    <div className="bg-white border border-neutral-200 hover:border-neutral-400 transition-colors" style={{
      borderLeftWidth: '4px',
      borderLeftColor: ente.status === 'updated' ? '#000' : '#d4d4d4'
    }}>
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 space-y-2">
            <div className="flex items-center gap-3 flex-wrap">
              <h3 className="text-base font-semibold text-black">{ente.name}</h3>
              ...
              <Button
                variant="ghost"
                size="sm"
                className={`h-7 px-2 ${ente.enabled !== false ? 'text-green-600 hover:text-green-700 hover:bg-green-50' : 'text-neutral-400 hover:text-neutral-600 hover:bg-neutral-50'}`}
                onClick={async () => {
                  try {
                    const response = await fetch(`http://localhost:3000/rest/v1/entes_publicos?id=eq.${ente.id}`, {
                      method: 'PATCH',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({ enabled: ente.enabled === false ? true : false })
                    });
                    if (response.ok) {
                      ente.enabled = ente.enabled === false ? true : false;
                      window.location.reload();
                    }
                  } catch (err) {
                    console.error('Error toggling ente:', err);
                  }
                }}
                title={ente.enabled !== false ? 'Desactivar monitoreo del ente' : 'Activar monitoreo del ente'}
              >
                <Power className="h-4 w-4 mr-1" />
                {ente.enabled !== false ? 'Activo' : 'Inactivo'}
              </Button>
            </div>
        ...
        <div className="space-y-2 border-t border-neutral-100 pt-3">
          ...
          {isEditingUrl ? (
            ...
            <div className="flex items-center gap-2 text-sm text-neutral-600">
              <span className="truncate flex-1">{ente.url}</span>
              <Button
                variant="ghost"
                size="icon"
          ...
          <div className="space-y-2 border-t border-neutral-100 pt-3">
            ...
            >
              <span className="text-sm font-medium text-black">
                Subenlaces monitorizados ({ente.sublinks.filter(s => s.enabled).length})
              </span>
